...
class LineFollower(Node):
	...
	def edge_vectors_callback(self, message):
		global turn_global,iteration, turn_updated,objects,upper_angle,single_vector,dist,speed, left_min,right_min,right_min_index,left_min_index,middle_angle,Threshold_safe,Threshold_Danger,left_under_threshold,right_under_threshold
		...
		if self.obstacle_detected is True:
			...
			angle_max = default_angle - middle_angle
			angle_band = 10
			if angle_max > 0:
				angle_max = angle_max + angle_band
			else:
				angle_max = angle_max - angle_band
			turn = turn_change(change,turn_global,-(angle_max*PI/180),dir)
			# turn = -((default_angle - middle_angle)*PI/180)
			turn_global = turn
			print(self.obstacle_status ,change, turn,-(angle_max*PI/180),middle_angle," Obstacle",iteration)
